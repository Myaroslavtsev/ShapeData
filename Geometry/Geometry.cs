...
namespace ShapeData.Geometry
{
    class Geometry
    {
        ...
        public static List<(double U, double V)> MakeSomeUVcoords(List<Point> points) =>        
            ScaleToUnitSquare(ProjectPointsToPlane(points));        

        private static List<(double U, double V)> ProjectPointsToPlane(List<Point> points)
        {
            ...
            for (int i = 3; i < points.Count; i++)
            {
                ...
                var distanceToPlane = Vector3.Dot(vecToP, normal);
                var projection = p - distanceToPlane * normal;
                var projectedVec = projection - origin;

                // Координаты в системе U-V
                double u = Vector3.Dot(projectedVec, uDir);
                double v = Vector3.Dot(projectedVec, vDir);
